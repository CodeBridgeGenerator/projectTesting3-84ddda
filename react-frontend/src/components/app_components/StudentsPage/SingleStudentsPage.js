...
const SingleStudentsPage = (props) => {
  ...
  const [isHelpSidebarVisible, setHelpSidebarVisible] = useState(false);
    ...
    useEffect(() => {
        ...
        client
            .service("students")
            .get(urlParams.singleStudentsId, { query: { $populate: [            {
                path: "createdBy",
                service: "users",
                select: ["name"],
              },{
    ...
    }, [props,urlParams.singleStudentsId]);
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Contact</label><p className="m-0 ml-3" >{Number(_entity?.contact)}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Is active</label><p className="m-0" ><i id="isActive" className={`pi ${_entity?.isActive?"pi-check": "pi-times"}`}  ></i></p></div>
      ...
      <CommentsSection
        recordId={urlParams.singleStudentsId}
        user={props.user}
        alert={props.alert}
