...
const StudentsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,address: _entity?.address,contact: _entity?.contact,dob: _entity?.dob,user: _entity?.user?._id,isActive: _entity?.isActive || false,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="contact" className="w-full mb-3 p-inputtext-sm" value={_entity?.contact} onChange={(e) => setValByKey("contact", e.value)}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="dob"  value={_entity?.dob ? new Date(_entity?.dob) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("dob", new Date(e.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["dob"]) ? (
